...
## Developer runbook: after code changes (target server folder workflow)

If you want to run Trino from `core/trino-server/target/trino-server-480-SNAPSHOT`, use the following steps in order.

1. Build and test the StarRocks plugin module:

```bash
cd /Users/stevenchung/Documents/Work/Energywell/trino
./mvnw -pl plugin/trino-starrocks test -DskipITs -DfailIfNoTests=false
./mvnw -pl plugin/trino-starrocks -DskipTests package
```

2. Rebuild the target server distribution so the latest plugin artifact is included:

```bash
cd /Users/stevenchung/Documents/Work/Energywell/trino
./mvnw -pl core/trino-server -am -DskipTests package
```

3. Copy your StarRocks catalog config into the target server `etc/catalog` folder:

```bash
cd /Users/stevenchung/Documents/Work/Energywell/trino
cp testing/trino-server-dev/etc/catalog/starrocks.properties \
	core/trino-server/target/trino-server-480-SNAPSHOT/etc/catalog/starrocks.properties
```

4. Start Trino from the target server folder:

```bash
cd /Users/stevenchung/Documents/Work/Energywell/trino/core/trino-server/target/trino-server-480-SNAPSHOT
bin/launcher run
```

5. Verify startup and connector availability:

```bash
curl -s http://localhost:8080/v1/info
```

6. Stop Trino when done:
...
- If only `starrocks.properties` changes and no code changes, you can skip Steps 1 and 2 and do Steps 3 and 4.
- If code changed, do all steps.
