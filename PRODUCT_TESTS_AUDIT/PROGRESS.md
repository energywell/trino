...
- Do not stop or claim completion until every lane and every suite in this file is marked `complete`, unless a true blocker is recorded in `GAPS.md`.
...
- Known unresolved fidelity gaps currently recorded in `GAPS.md`: none.
...
- If a blocker exists, it must be written in `GAPS.md` and the exact stopped state must be written here before stopping.
